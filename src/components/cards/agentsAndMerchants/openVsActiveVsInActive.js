...
export default function OpenVsActiveVsInActive({ fulldata }) {
            ...
            <Stack direction="column" spacing={1} sx={{ mb: 1, mt: 2, width: '100%', height: '100%', alignItems: 'flex-start' }}>
                <Stack direction='row' spacing={3} sx={{ justifyContent: 'space-between' }}>
                    <Stack direction='column' sx={{ justifyContent: 'space-between' }}>
                        <Typography variant="h6" sx={{ maxWidth: '200px' }}>{fulldata.openVsActiveVsInActive.head}</Typography>
                        <Typography sx={{ fontSize: '12px' }}>{fulldata.openVsActiveVsInActive.duration}</Typography>
                ...
                <PieChart
                    ...
                    series={[
                        {
                            data: fulldata.openVsActiveVsInActive.data.map((item) => ({ label: item.label, value: item.value ,   color: item.color  })),
                            innerRadius: 80,
                            outerRadius: 50,
                            cornerRadius: 0,
                            paddingAngle: 0,
                ...
                <Stack spacing={1} direction='row' sx={{ mt: 2, py: 2, width: '100%', justifyContent: 'space-around' }}>
                    {fulldata.openVsActiveVsInActive.data.map((item) => (
